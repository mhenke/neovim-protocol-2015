...
- Key sequences required for tasks must be logical and efficient, reflecting how a real user would perform the action. Redundant or nonsensical sequences (e.g., `gg, 0`) are forbidden.
...
## 0. Curriculum-to-Task Correspondence
- **Principle:** Every command listed under a level's "Core Skill" in `LEVEL.md` must have at least one corresponding, practical task in the `constants_static.ts` implementation.
- **Validation:** Before finalizing a level, audit `constants_static.ts` against `LEVEL.md`. If a core skill is listed in the design doc, a task *must* exist to test it. There is no "false advertising".
- **Rationale:** This closes the gap between design and implementation, ensuring the student practices every skill we claim to teach.
